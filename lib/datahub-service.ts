  ...
  try {
    ...
    const datasetData = {
      ...
      metadata: {
        ...
        autoGenerated: true,
        linkedFormId: formId,
        isPublic: true, // Make auto-created datasets public so they can be viewed in DataHub
      },
      createdAt: Timestamp.fromDate(now),
    ...
    console.log(`✅ Created dataset ${docRef.id} for form ${formId}`);
    return docRef.id;
  } catch (error) {
...
export async function updateDatasetSchema(
  ...
  try {
    ...
    console.log(`✅ Updated schema for dataset ${datasetId}`);
  } catch (error) {
...
export async function linkFormToDataset(
  ...
  try {
    ...
    console.log(`✅ Linked form ${formId} to dataset ${datasetId}`);
  } catch (error) {
...
export async function ensureDatasetForForm(
  ...
  try {
    ...
    if (datasetId) {
      console.log(`📊 Using existing dataset ${datasetId} for form ${formId}`);
      return datasetId;
    }
    
    // Create new dataset
    console.log(`🆕 Creating new dataset for form ${formId}`);
...
export async function deleteDataset(datasetId: string): Promise<void> {
  try {
    ...
    console.log(`🗑️ Deleted dataset ${datasetId} and ${snapshot.size} records`);
  } catch (error) {
...
export async function unlinkFormFromDataset(formId: string): Promise<void> {
  try {
    ...
    console.log(`🔗 Unlinked form ${formId} from dataset`);
  } catch (error) {
