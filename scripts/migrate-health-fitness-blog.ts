/**
 * Script to migrate the "Health and Fitness for Youth" blog article to Firestore
 * Run this once to add the article to the database
 */
...
async function migrateBlog() {
  try {
    console.log('Migrating "Health and Fitness for Youth" blog to Firestore...');
    
    const docRef = await addDoc(collection(db, 'blogEntries'), healthFitnessBlog);
    
    console.log('✅ Blog article successfully migrated!');
    console.log('Document ID:', docRef.id);
    console.log('Article URL: http://localhost:3000/blog/health-and-fitness-for-youth');
    console.log('\nYou can now manage this article in the admin panel at:');
    console.log('http://localhost:3000/admin/blog-entries');
    
  } catch (error) {
    console.error('❌ Error migrating blog:', error);
    console.log('\nMake sure:');
    console.log('1. Firebase rules are deployed');
    console.log('2. You are logged in as an admin');
    console.log('3. The blogEntries collection exists');
  }
}
